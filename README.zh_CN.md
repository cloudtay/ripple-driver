...
> 该项目为主流PHP框架提供了使用PRipple驱动的能力, 能使得原有项目在不修改项目代码的前提下以CLI模式运行, 加速框架的运行速度,
> 达到编译级性能(约传统FPM的10~40倍),适用于高并发场景
...
#### Laravel & ThinkPHP 项目差异

> ⚠️在`Laravel`,`ThinkPHP下`的CLI模式, 整个运行过程的 `Controller` `Service`
> 等 `Container` 构建的单例,只会在运行时被构造一次(全局唯一控制器对象), 且在整个运行过程中不会被销毁
> 在开发过程应特别关心这一点, Request应该具体到某个Action的参数上以确保线程安全,
> CLI模式在这点上与FPM截然不同, 但这也是它能够拥有火箭般速度的原因之一

> ⚠️你需要有一定了解CLI运行模式的机制,并知悉下列函数在运行过程中会发生什么以决定如何使用它们?如
